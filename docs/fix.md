...
TODO: introduce the FIX protocol
...
First things first: we need to set our separator. Since both examples provided above were using the pipe character "|" as the separator, we will be using that exact one. Keep in mind that the FIX protocol itself uses Start of Heading ([SoH](https://www.ascii-code.com/1)). The pipe character is only used for representing the whole string since SoH isn't printable. This will have an effect on the examples we will be using for testing our pattern, since the checksum will need to be adjusted. On the "Logon" message, the checksum will become `145` instead of `062` and on the "Execution Report" we will have a checksum of `008` instead of `128`. In essence, the examples we will be using from this point onward will be this ones:
...
Apart from this, we need to create the field ID to field name dictionary and either one of those to the final type the field will be casted to. This can be done with the following lines of code:
...
> This is a sample that only covers the fields used on these two examples we are covering. For a full implementation, these dictionaries would be much larger.
...
```q
(seq/)(map[{dis[x]}]j;skip chr"=";many1[noneof[enlist separator]])
```

Where we:
    1. Detect the field ID, cast it into an integer and convert into the field name.
...
```q
q)tuple:(seq/)(map[{dis[x]}]j;skip chr"=";many1[noneof[enlist separator]]);
q)show tuple"8=FIX.4.2";
...
This is a really nice start if you ask me, but we could do even better. By using the `seqf` combinator instead of `seq`, we can apply a function to our pattern. By exploiting this, we should be able to transform this nested list into the much-needed tuple:
...
```q
q)tuple:(seqf[translate]/)(map[{dis[x]}]j;skip chr"=";many1[noneof[enlist separator]]);
q)show tuple"8=FIX.4.2";
...
Using this `tuple` pattern, we can then match N tuples. In order to do that, since we know that we have a set separator, we can leverage the `sep` combinator, which takes two patterns as inputs: the pattern that will be repeated and then the separator pattern. In our case, the first one will be `tuple`, whereas the second one will be simply `chr separator`, defined above.

We also need to take into consideration that at the end of our FIX message there will be an extra separator that delimits the end of the message. We can take this into consideration using the following syntax:

```q
fixformat:(seq/)(sep[tuple;chr separator];skip chr separator;eof);
```

Once again this will bring some noise to our output. In order to get rid of it, we can once again use `seqf` to "bubble up" the desired output:

```q
q)fixformat:(seqf[(,).]/)(sep[tuple;chr separator];skip chr separator;eof);
q)show fixformat fixmsg;
...
Now that we have our core parser built, we can move onto validating the checksum. For that, we will need to do some work _before_ and _after_ our parser evaluates. First we will need to calculate our checksum, then parse, then match that first result to the one found on the message itself.

#### Calculating the checksum

In order to calculate the checksum, we should apply this function to our message:

```q
checkSumCalc:{(sum -7 _x)mod 256};
```

We first get rid of the `CheckSum` field altogether (the last seven characters) and sum all the other characters. After that, we perform the modulo on that result and that's the result of our chechsum.

As you can probably guess, this should be done _before_ parsing because we are not only parsing but transforming as well. In order to achieve this, we can use `map` and `mget` (TODO: explain `mget`):

```q
q)show map[checkSumCalc][mget] fixmsg;
...
```q
q)fixp:seq[map[checkSumCalc][mget];fixformat];
q)fixp fixmsg;
...
We get our list of touples instead of the unparsed string. With this in mind, we can later on extract the `CheckSum` field and compare against it. Before that however, it would be nice to turn our list of tuples into an actual dictionary. We can create a function `todict` that looks like this:

```q
todict:{(first x;(!). flip last x)};
```

Which then we can run on our parsed result using `map`:

```q
q)fixp:map[todict] seq[map[checkSumCalc][mget];fixformat];
q)fixp fixmsg;
...
```q
q)checkSumMatch:{first[x]~last[x][`CheckSum]};
```

Keep in mind that `x`'s first element is the checksum value and its last is the dictionary.

Now, using `fil`, we can perform the check which if unsuccesful will drop everything we have until this point:

```q
q)fixp:fil[checkSumMatch]
     map[todict]
